...
hdu_1808_j= fits.open('/home/pierre/Documents/2021/SINFONI_analysis/SINFONI_CALIB/NGC_7552/J.fits')
hdu_1808_h = fits.open('/home/pierre/Documents/2021/SINFONI_analysis/SINFONI_CALIB/NGC_7552/H.fits')
hdu_1808_k = fits.open('/home/pierre/Documents/2021/SINFONI_analysis/SINFONI_CALIB/NGC_7552/K.fits')
...
plt.axis([1.11,2.45,0,5e-14])



a18, b18, c18, d18, mask18 = ngc1808_j.fit_all_gauss(1.288,1.287,1.291)
ab18, bb18, cb18, db18, maskb18= ngc1808_k.fit_all_gauss(2.178,2.176,2.180)
...
a18, b18, c18, d18, mask18 = ngc1808_j.fit_all_gauss(1.28,1.275,1.286)
ab18, bb18, cb18, db18, maskb18= ngc1808_k.fit_all_gauss(2.170,2.165,2.175)
ext1808 =  11.52*np.log10(5.89/(a18/ab18))
...
plt.figure()
# plt.imshow(map1808, vmin=0, vmax=5, cmap='jet')
plt.imshow(map1808, origin='lower', vmin=0, vmax=10, cmap='jet')
plt.xlabel('Relative Right Ascension (")')
plt.ylabel('Relative Declination (")')
cbar = plt.colorbar()
cbar.set_label('$A_v$')
...
ah218, bh218, ch218, dh218, maskh218 = ngc1808_k.fit_all_gauss(2.130,2.125,2.140)

plt.figure()
map_h2flux = np.ma.masked_array(scipy.signal.medfilt(10**(0.4*A_l(2.12,ext1808))*ah218*(np.pi/ch218)**0.5,(3,3)), mask1808)
...
map_h2hotmass = np.ma.masked_array(map_h2hotmass,mask1808)
plt.imshow(np.ma.masked_array(scipy.signal.medfilt(map_h2hotmass),mask1808), vmin=0,vmax=500,cmap='jet', origin='lower')
plt.xlabel('Relative Right Ascension (")')
plt.ylabel('Relative Declination (")')
cbar = plt.colorbar()
...
map_h2speed = scipy.signal.medfilt(c*(map_h2lam/z-2.12228)/2.12228,(3,3))
map_h2speed = np.ma.masked_array(map_h2speed, mask1808)
plt.figure()
plt.imshow(map_h2speed*1e-3, cmap='jet', origin='lower', vmin=-100, vmax=100, extent=[np.min(ngc1808_k.RA), np.max(ngc1808_k.RA), np.min(ngc1808_k.dec), np.max(ngc1808_k.dec)])
plt.xlabel('Relative Right Ascension (")')
plt.ylabel('Relative Declination (")')
cbar = plt.colorbar()
...
mesh = np.meshgrid(ngc1808_k.RA*276, ngc1808_k.dec*276 )
m = mask1808
pf, cf = curve_fit(plummer_full, [mesh[0][m==0],mesh[1][m==0]], map_h2speed[m==0], p0 = [0, 1e4, 100, 50*np.pi/180, 10*np.pi/180, 0, 0], maxfev=100000)

p, c = curve_fit(plummer, [mesh[0][m==0],mesh[1][m==0]], map_h2speed[m==0], p0 = [0, 1e4, 100, 50*np.pi/180, 0*np.pi/180], maxfev=100000)

plt.figure()
plt.imshow(np.reshape(plummer_full(mesh, *pf)*1e-3, np.shape(mesh[0])), cmap='jet', origin='lower', vmin=-100, vmax=100, extent=[np.min(ngc1808_k.RA), np.max(ngc1808_k.RA), np.min(ngc1808_k.dec), np.max(ngc1808_k.dec)])
plt.xlabel('Relative Right Ascension (")')
plt.ylabel('Relative Declination (")')
cbar = plt.colorbar()
cbar.set_label('Velocity ($km.s^{-1}$)')
...
afe218, bfe218, cfe218, dfe218, maskfe18 = ngc1808_h.fit_all_gauss(1.642, 1.63, 1.651)
...
plt.figure()
plt.imshow(1e5*snr, origin='lower', vmin=0, vmax=100, extent=[np.min(ngc1808_h.RA), np.max(ngc1808_h.RA), np.min(ngc1808_h.dec), np.max(ngc1808_h.dec)])
plt.xlabel('Relative Right Ascension (")')
plt.ylabel('Relative Declination (")')
cbar = plt.colorbar()
...
def manypng(filename, figs=None, dpi=200):
    ...
    for fig in figs:
        ...
        k+=1
multipage('/home/pierre/Documents/2021/SINFONI_analysis/SINFONI_CALIB/NGC_7552/ANALYSIS/plots.pdf')
manypdf('/home/pierre/Documents/2021/SINFONI_analysis/SINFONI_CALIB/NGC_7552/ANALYSIS/plot')
manypng('/home/pierre/Documents/2021/SINFONI_analysis/SINFONI_CALIB/NGC_7552/ANALYSIS/plot')
plt.close('all')