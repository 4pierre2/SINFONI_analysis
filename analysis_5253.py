...
hdu_1808_j= fits.open('/home/pierre/Documents/2021/SINFONI_analysis/SINFONI_CALIB/NGC_5253/J.fits')
hdu_1808_h = fits.open('/home/pierre/Documents/2021/SINFONI_analysis/SINFONI_CALIB/NGC_5253/H.fits')
hdu_1808_k = fits.open('/home/pierre/Documents/2021/SINFONI_analysis/SINFONI_CALIB/NGC_5253/K.fits')
...
plt.axis([1.11,2.45,0,1.15e-14])

a18, b18, c18, d18, mask18 = ngc1808_j.fit_all_gauss(1.285,1.280,1.290)
ab18, bb18, cb18, db18, maskb18= ngc1808_k.fit_all_gauss(2.168,2.165,2.175)
...
plt.figure()
plt.imshow(map1808, origin='lower', vmin=-2, vmax=6, cmap='jet')
...
plt.figure()
h2flux = 10**(0.4*A_l(2.12,ext1808))*ah218*(np.pi/ch218)**0.5
mask1808_h2flux = deepcopy(mask1808)
mask1808_h2flux[h2flux>5e-20]=1
map_h2flux = np.ma.masked_array(scipy.signal.medfilt(h2flux, (3,3)), mask1808_h2flux)
plt.imshow(map_h2flux, cmap='jet', vmin = 8e-22, vmax=3e-20, origin='lower', norm=LogNorm())
plt.xlabel('Relative Right Ascension (")')
plt.ylabel('Relative Declination (")')
cbar = plt.colorbar()
...
# plt.figure()
a18, b18, c18, d18, mask18 = ngc1808_j.fit_all_gauss(1.285,1.280,1.290)
map_h2lam = np.ma.masked_array(b18, mask1808)
# img = plt.imshow(map_h2lam, cmap='jet', vmin = 2.1282, vmax=2.1302, origin='lower')
map_h2speed = c*(map_h2lam/z-1.28181)/1.28181
plt.figure()
plt.imshow(map_h2speed*1e-3, cmap='jet', origin='lower', vmin=30, vmax=100, extent=[np.min(ngc1808_k.RA), np.max(ngc1808_k.RA), np.min(ngc1808_k.dec), np.max(ngc1808_k.dec)])
plt.xlabel('Relative Right Ascension (")')
plt.ylabel('Relative Declination (")')
cbar = plt.colorbar()
...
# as18, bs18, cs18, ds18, masks18 = ngc1808_j.fit_all_gauss_abs(1.213,1.210,1.215)

# map_slam = np.ma.masked_array(bs18, mask1808)
# map_sspeed = c*(map_slam/z-1.213)/1.213
# plt.figure()
...
mask1808_snr = deepcopy(mask1808)
mask1808_snr [snr>1e-5] = 1
snr = np.ma.masked_array(snr, mask1808_snr)
plt.figure()
plt.imshow(1e5*snr, origin='lower', vmin=0, vmax=0.35, extent=[np.min(ngc1808_h.RA), np.max(ngc1808_h.RA), np.min(ngc1808_h.dec), np.max(ngc1808_h.dec)])
plt.xlabel('Relative Right Ascension (")')
plt.ylabel('Relative Declination (")')
cbar = plt.colorbar()
...
multipage('/home/pierre/Documents/2021/SINFONI_analysis/SINFONI_CALIB/NGC_5253/ANALYSIS/plots.pdf')
manypdf('/home/pierre/Documents/2021/SINFONI_analysis/SINFONI_CALIB/NGC_5253/ANALYSIS/plot')
manypng('/home/pierre/Documents/2021/SINFONI_analysis/SINFONI_CALIB/NGC_5253/ANALYSIS/plot')
plt.close('all')